...
export class SupportService {
  ...
  public static async createTicket(
    ...
    data: {
      orderId?: string | null;
      category: 'ORDER_ISSUE' | 'PAYMENT_ISSUE' | 'DELIVERY_ISSUE' | 'GENERAL';
      subject: string;
      description: string;
      priority?: 'LOW' | 'MEDIUM' | 'HIGH' | 'URGENT';
    }
  ): Promise<SupportTicketRecord> {
    // If orderId provided, verify order belongs to user or user is authorized
    if (data.orderId) {
      const orderCheck = await query(
        `SELECT id FROM order_management.orders WHERE id = $1`,
        [data.orderId]
      );
    ...
    const res = await query<any>(
      `INSERT INTO support.tickets (
         ticket_number, user_id, order_id, category, subject, description, priority, status
